...
class RealImgDataset(Dataset):
    def __init__(self, lq_path, meta_info_file=None, image_size=256):
        self.lq_folder = lq_path
    ...
    def __getitem__(self, idx):
        lq_path = self.paths[idx]
        img = cv2.imread(lq_path)
        while img is None:
            print('Bad image {}...'.format(lq_path))
        ...
        return {'img': img, 'name': os.path.basename(lq_path)}


if __name__ == "__main__":
    path = 'BasicSR/datasets/vlink/val/lr10'
    dataset = RealImgDataset(path) 
    assert dataset.meta_info_file == 'BasicSR/basicsr/data/meta_info/meta_info_DF2K_sub_GT.txt'
    
    for i in range(10):
        result = dataset.__getitem__(i)
        print(f"result: {result['img'].shape}, name: {result['name']}")
